    ...
    <div class="dialog-cont">
      ...
      <div class="dialog-tab-cont" v-if="!tabShow">
        <div class="show-title">
          <font>显示标题:</font>
          <label>
            <input type="radio" name="showTitle" v-model="linkdata.show" value="1" @click="linkdata.show=1">不显示
          </label>
          <label>
            <input type="radio" name="showTitle" v-model="linkdata.show" value="2" @click="linkdata.show=2">显示
          </label>
          <div class="set-title" v-if="linkdata.show==2">
  ...
  methods: {
    ...
    re_selection: function() {
      ...
      if (site.location1 == "hd") {
        newlinksdata = JSON.parse(
          JSON.stringify(that.layout_data.hd[site.location4].data)
        );
      }
      if (site.location1 == "ft") {
        newlinksdata = JSON.parse(
          JSON.stringify(that.layout_data.ft[site.location4].data)
        );
      ...
      if (site.location1 == "con") {
        if (site.location3 == "w19") {
          newlinksdata = JSON.parse(
            JSON.stringify(
              that.layout_data.con[site.location2].w19[site.location4].data
            )
          );
        }
        if (site.location3 == "w75") {
          newlinksdata = JSON.parse(
            JSON.stringify(
              that.layout_data.con[site.location2].w75[site.location4].data
            )
          );
        }
        if (site.location3 == "center") {
          newlinksdata = JSON.parse(
            JSON.stringify(
              that.layout_data.con[site.location2].w1920[site.location4].data
            )
          );
        }
      }
      console.log(newlinksdata.linklist);
