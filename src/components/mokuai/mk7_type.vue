        ...
        <div class="link-words" v-if="radioShow==true">
          <ul>
            ...
            <li v-for="(item,index) in linklist">
              <span>
                链接名称:
                <input type="text" class="input1" :ref="'name'+index" :value="item.name">
                <div class="dialog-operation" @mouseleave="item.display=false">
                  <a @mouseenter="item.display=true" id="move">移动
                    <div class="move" v-show="item.display">
                      <span @click="moveall(-1,index);item.display=false">上一排</span>
              ...
              <span>
                链接地址:
                <input type="text" class="input2" :ref="'urlsite'+index" :value="item.urlsite">
              </span>
              <span>
                链接说明:
                <input
                  type="text"
                  class="input2"
                  :ref="'descration'+index"
                  :value="item.descration"
                >
              </span>
            </li>
          </ul>
  ...
  methods: {
    ...
    savedata() {
      ...
      for (var i = 0; i < 10000; i++) {
        var name = "name" + i;
        if (that.$refs[name]) {
          that.linklist.name =that.$refs[name][0].value;
        } else {
          return;
        }
      }
        let data={
            name: that.linklist.name,
            urlsite: null,
            descration: null,
            display: false
         }
